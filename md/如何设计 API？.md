> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [mp.weixin.qq.com](https://mp.weixin.qq.com/s/erSZglyGhj1o2HNzsgfxJQ)

> 在前后端分离的设计中，不管使用什么语言，后端都需要提供 WebAPI 给前端使用。如果是一个平台级的产品，还

![](https://mmbiz.qpic.cn/mmbiz_jpg/eqcwh4cqlibFibTiaYskqKPcwjz1baDR5z9Iru9rQNZyMOej51nSB9UdF1BviaKxpgYqSjBJ8SaVP5o33fKwQWvdjg/640?wx_fmt=jpeg)
...
要解决这些问题，就应该单独提供一个独立的公共 API，而不是直接让第三方开发人员或其他客户端直接访问平台公开的 API ，涉及到独立的公共 API公共api指整合了服务器内基础api的复合api。 举个例子，当请求edoc2服务器想要获取文档列表中每一个文档的属性信息时，可能需要进行多次api请求：请求文档列表、请求文档的属性信息。为此，客户端开发人员可能会在客户端中自己对这些请求进行组合，合并为一个服务，但是这种情况会导致逻辑不可控，不利于未来维护。另一种方法是服务器提供复合的api，这就使用到了所谓的公共api。开发人员当然也可以考虑把这个内容增加为一个新的基础api，但是这会导致api爆炸，所以就需要使用到api网关。，API 网关就要出场了。
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/eqcwh4cqlibFibTiaYskqKPcwjz1baDR5z9hFMXUuPLaWAS5DoJDkQqafe1vM9kxqbyBpMMTicZHuFbpZr98G74Aibw/640?wx_fmt=other)image-20231016161047257
...
Ocelot：是一个基于. NET Core 的轻量级 API 网关，用于构建和管理微服务架构中的 API 网关。作为一个开源项目，Ocelot 提供了一种灵活、可扩展的方式来集中处理请求路由、认证授权、请求转发、负载均衡和缓存等功能。
...
Envoy：是一个开源的高性能代理和通信中间件，专为云原生应用程序设计。它由 Lyft 开发并于 2017 年成为 Cloud Native Computing Foundation（CNCF）的毕业项目之一。虽然 Envoy 本身是用 C++ 编写的，但它可以与任何语言和框架进行集成，包括 .NET Core。
